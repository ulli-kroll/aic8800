#include "siwifi_defs.h"
#include "siwifi_msg_tx.h"
#include "siwifi_events.h"
void siwifi_mu_group_sta_init(struct siwifi_sta *sta,
                            const struct ieee80211_vht_cap *vht_cap)
{
    sta->group_info.map = 0;
    sta->group_info.cnt = 0;
    sta->group_info.active.next = LIST_POISON1;
    sta->group_info.update.next = LIST_POISON1;
    sta->group_info.last_update = 0;
    sta->group_info.traffic = 0;
    sta->group_info.group = 0;
    if (!vht_cap ||
        !(vht_cap->vht_cap_info & IEEE80211_VHT_CAP_MU_BEAMFORMEE_CAPABLE)) {
            sta->group_info.map = SIWIFI_SU_GROUP;
    }
}
void siwifi_mu_group_sta_del(struct siwifi_hw *siwifi_hw, struct siwifi_sta *sta)
{
    struct siwifi_mu_info *mu = &siwifi_hw->mu;
    int i, j, group_id;
    bool lock_taken;
    u64 map;
    lock_taken = (down_interruptible(&mu->lock) == 0);
    group_sta_for_each(sta, group_id, map) {
        struct siwifi_mu_group *group = siwifi_mu_group_from_id(mu, group_id);
        for (i = 0; i < CONFIG_USER_MAX; i++) {
            if (group->users[i] == sta) {
                group->users[i] = NULL;
                group->user_cnt --;
                if (group->user_cnt == 1) {
                    for (j = 0; j < CONFIG_USER_MAX; j++) {
                        if (group->users[j]) {
                            group->users[j]->group_info.cnt--;
                            group->users[j]->group_info.map &= ~BIT_ULL(group->group_id);
                            if (group->users[j]->group_info.group == group_id)
                                group->users[j]->group_info.group = 0;
                            group->user_cnt --;
                            break;
                        }
                    }
                    mu->group_cnt--;
                    trace_mu_group_delete(group->group_id);
                } else {
                    trace_mu_group_update(group);
                }
                break;
            }
        }
        WARN((i == CONFIG_USER_MAX), "sta %d doesn't belongs to group %d",
            sta->sta_idx, group_id);
    }
    sta->group_info.map = 0;
    sta->group_info.cnt = 0;
    sta->group_info.traffic = 0;
    if (sta->group_info.active.next != LIST_POISON1)
        list_del(&sta->group_info.active);
    if (sta->group_info.update.next != LIST_POISON1)
        list_del(&sta->group_info.update);
    if (lock_taken)
        up(&mu->lock);
}
u64 siwifi_mu_group_sta_get_map(struct siwifi_sta *sta)
{
    if (sta)
        return sta->group_info.map;
    return 0;
}
int siwifi_mu_group_sta_get_pos(struct siwifi_hw *siwifi_hw, struct siwifi_sta *sta,
                              int group_id)
{
    struct siwifi_mu_group *group;
    int i;
    group = siwifi_mu_group_from_id(&siwifi_hw->mu, group_id);
    if (!group)
        return -1;
    for (i = 0; i < CONFIG_USER_MAX; i++) {
        if (group->users[i] == sta)
            return i;
    }
    WARN(1, "sta %d doesn't belongs to group %d",
         sta->sta_idx, group_id);
    return -1;
}
static inline
void siwifi_mu_group_move_head(struct list_head *list, struct list_head *elem)
{
    if (elem->next != LIST_POISON1) {
        __list_del_entry(elem);
    }
    list_add(elem, list);
}
static inline
void siwifi_mu_group_remove_users(struct siwifi_mu_info *mu,
                                struct siwifi_mu_group *group)
{
    struct siwifi_sta *sta;
    int i, group_id = group->group_id;
    for (i = 0; i < CONFIG_USER_MAX; i++) {
        if (group->users[i]) {
            sta = group->users[i];
            group->users[i] = NULL;
            sta->group_info.cnt--;
            sta->group_info.map &= ~BIT_ULL(group_id);
            siwifi_mu_group_move_head(&mu->update_sta,
                                    &sta->group_info.update);
        }
    }
    if (group->user_cnt)
        mu->group_cnt--;
    group->user_cnt = 0;
    trace_mu_group_delete(group_id);
}
static inline
void siwifi_mu_group_add_users(struct siwifi_mu_info *mu,
                             struct siwifi_mu_group *group,
                             int nb_user, struct siwifi_sta **users)
{
    int i, j, group_id = group->group_id;
    if (!group->user_cnt)
        mu->group_cnt++;
    j = 0;
    for (i = 0; i < nb_user ; i++) {
        for (; j < CONFIG_USER_MAX ; j++) {
            if (group->users[j] == NULL) {
                group->users[j] = users[i];
                users[i]->group_info.cnt ++;
                users[i]->group_info.map |= BIT_ULL(group_id);
                siwifi_mu_group_move_head(&(mu->update_sta),
                                        &(users[i]->group_info.update));
                group->user_cnt ++;
                j ++;
                break;
            }
            WARN(j == (CONFIG_USER_MAX - 1),
                 "Too many user for group %d (nb_user=%d)",
                 group_id, group->user_cnt + nb_user - i);
        }
    }
    trace_mu_group_update(group);
}
static
int siwifi_mu_group_create_one(struct siwifi_mu_info *mu, int nb_user,
                             struct siwifi_sta **users, int *nb_group_left)
{
    int i, group_id;
    struct siwifi_mu_group *group;
    u64 group_match;
    u64 group_avail;
    group_match = users[0]->group_info.map;
    group_avail = users[0]->group_info.map;
    for (i = 1; i < nb_user ; i++) {
        group_match &= users[i]->group_info.map;
        group_avail |= users[i]->group_info.map;
    }
    if (group_match) {
        group_id = SIWIFI_GET_FIRST_GROUP_ID(group_match);
        group = siwifi_mu_group_from_id(mu, group_id);
        siwifi_mu_group_move_head(&mu->active_groups, &group->list);
        return 0;
    }
#if CONFIG_USER_MAX > 2
    if (group_avail) {
        struct siwifi_sta *users2[CONFIG_USER_MAX];
        int nb_user2;
        group_for_each(group_id, group_avail) {
            group = siwifi_mu_group_from_id(mu, group_id);
            if (group->user_cnt == CONFIG_USER_MAX)
                continue;
            nb_user2 = 0;
            for (i = 0; i < nb_user ; i++) {
                if (!(users[i]->group_info.map & BIT_ULL(group_id))) {
                    users2[nb_user2] = users[i];
                    nb_user2++;
                }
            }
            if ((group->user_cnt + nb_user2) <= CONFIG_USER_MAX) {
                siwifi_mu_group_add_users(mu, group, nb_user2, users2);
                siwifi_mu_group_move_head(&mu->active_groups, &group->list);
                return 0;
            }
        }
    }
#endif
    group = list_last_entry(&mu->active_groups, struct siwifi_mu_group, list);
    siwifi_mu_group_remove_users(mu, group);
    siwifi_mu_group_add_users(mu, group, nb_user, users);
    siwifi_mu_group_move_head(&mu->active_groups, &group->list);
    (*nb_group_left)--;
    return 1;
}
static
void siwifi_mu_group_create(struct siwifi_mu_info *mu, struct siwifi_sta *sta,
                          int *nb_group_left)
{
    struct siwifi_sta *user_sta = sta;
    struct siwifi_sta *users[CONFIG_USER_MAX];
    int nb_user = 1;
    users[0] = sta;
    while (*nb_group_left) {
        list_for_each_entry_continue(user_sta, &mu->active_sta, group_info.active) {
            users[nb_user] = user_sta;
            if (++nb_user == CONFIG_USER_MAX) {
                break;
            }
        }
        if (nb_user > 1) {
            if (siwifi_mu_group_create_one(mu, nb_user, users, nb_group_left))
                (*nb_group_left)--;
            if (nb_user < CONFIG_USER_MAX)
                break;
            else
                nb_user = 1;
        } else
            break;
    }
}
void siwifi_mu_group_work(struct work_struct *ws)
{
    struct delayed_work *dw = container_of(ws, struct delayed_work, work);
    struct siwifi_mu_info *mu = container_of(dw, struct siwifi_mu_info, group_work);
    struct siwifi_hw *siwifi_hw = container_of(mu, struct siwifi_hw, mu);
    struct siwifi_sta *sta, *next;
    int nb_group_left = NX_MU_GROUP_MAX;
    if (WARN(!siwifi_hw->mod_params->mutx,
             "In group formation work, but mutx disabled"))
        return;
    if (down_interruptible(&mu->lock) != 0)
        return;
    mu->update_count++;
    if (!mu->update_count)
        mu->update_count++;
    list_for_each_entry_safe(sta, next, &mu->active_sta, group_info.active) {
        if (nb_group_left)
            siwifi_mu_group_create(mu, sta, &nb_group_left);
        sta->group_info.last_update = mu->update_count;
        list_del(&sta->group_info.active);
    }
    if (! list_empty(&mu->update_sta)) {
        list_for_each_entry_safe(sta, next, &mu->update_sta, group_info.update) {
            siwifi_send_mu_group_update_req(siwifi_hw, sta);
            list_del(&sta->group_info.update);
        }
    }
    mu->next_group_select = jiffies;
    siwifi_mu_group_sta_select(siwifi_hw);
    up(&mu->lock);
    return;
}
void siwifi_mu_group_init(struct siwifi_hw *siwifi_hw)
{
    struct siwifi_mu_info *mu = &siwifi_hw->mu;
    int i;
    INIT_LIST_HEAD(&mu->active_groups);
    INIT_LIST_HEAD(&mu->active_sta);
    INIT_LIST_HEAD(&mu->update_sta);
    for (i = 0; i < NX_MU_GROUP_MAX; i++) {
        int j;
        mu->groups[i].user_cnt = 0;
        mu->groups[i].group_id = i + 1;
        for (j = 0; j < CONFIG_USER_MAX; j++) {
            mu->groups[i].users[j] = NULL;
        }
        list_add(&mu->groups[i].list, &mu->active_groups);
    }
    mu->update_count = 1;
    mu->group_cnt = 0;
    mu->next_group_select = jiffies;
    INIT_DELAYED_WORK(&mu->group_work, siwifi_mu_group_work);
    sema_init(&mu->lock, 1);
}
void siwifi_mu_set_active_sta(struct siwifi_hw *siwifi_hw, struct siwifi_sta *sta,
                            int traffic)
{
    struct siwifi_mu_info *mu = &siwifi_hw->mu;
    if (!sta || (sta->group_info.map & SIWIFI_SU_GROUP))
        return;
    sta->group_info.traffic += traffic;
    if ((sta->group_info.last_update != mu->update_count) ||
        !list_empty(&mu->active_sta)) {
        siwifi_mu_group_move_head(&mu->active_sta, &sta->group_info.active);
        if (!delayed_work_pending(&mu->group_work) &&
            !list_is_singular(&mu->active_sta)) {
            schedule_delayed_work(&mu->group_work,
                                  msecs_to_jiffies(SIWIFI_MU_GROUP_INTERVAL));
        }
    }
}
void siwifi_mu_set_active_group(struct siwifi_hw *siwifi_hw, int group_id)
{
    struct siwifi_mu_info *mu = &siwifi_hw->mu;
    struct siwifi_mu_group *group = siwifi_mu_group_from_id(mu, group_id);
    siwifi_mu_group_move_head(&mu->active_groups, &group->list);
}
void siwifi_mu_group_sta_select(struct siwifi_hw *siwifi_hw)
{
    struct siwifi_mu_info *mu = &siwifi_hw->mu;
    int nb_users[NX_MU_GROUP_MAX + 1];
    int traffic[NX_MU_GROUP_MAX + 1];
    int order[NX_MU_GROUP_MAX + 1];
    struct siwifi_sta *sta;
    struct siwifi_vif *vif;
    struct list_head *head;
    u64 map;
    int i, j, update, group_id, tmp, cnt = 0;
    if (!mu->group_cnt || time_before(jiffies, mu->next_group_select))
        return;
    list_for_each_entry(vif, &siwifi_hw->vifs, list) {
        if (SIWIFI_VIF_TYPE(vif) != NL80211_IFTYPE_AP)
            continue;
        head = &vif->ap.sta_list;
        memset(nb_users, 0, sizeof(nb_users));
        memset(traffic, 0, sizeof(traffic));
        list_for_each_entry(sta, head, list) {
            int sta_traffic = sta->group_info.traffic;
            sta->group_info.traffic = 0;
            if (sta->group_info.group)
                trace_mu_group_selection(sta, 0);
            sta->group_info.group = 0;
            if (sta->group_info.cnt == 0 ||
                sta_traffic < SIWIFI_MU_GROUP_MIN_TRAFFIC)
                continue;
            group_sta_for_each(sta, group_id, map) {
                nb_users[group_id]++;
                traffic[group_id] += sta_traffic;
                if (nb_users[group_id] == 2)
                    order[cnt++] = group_id;
            }
        }
        update = 1;
        while(update) {
            update = 0;
            for (i = 0; i < cnt - 1; i++) {
                if (traffic[order[i]] < traffic[order[i + 1]]) {
                    tmp = order[i];
                    order[i] = order[i + 1];
                    order[i + 1] = tmp;
                    update = 1;
                }
            }
        }
        for (i = 0; i < cnt ; i ++) {
            struct siwifi_mu_group *group;
            group_id = order[i];
            if (nb_users[group_id] < 2)
                continue;
            group = siwifi_mu_group_from_id(mu, group_id);
            for (j = 0; j < CONFIG_USER_MAX ; j++) {
                if (group->users[j]) {
                    trace_mu_group_selection(group->users[j], group_id);
                    group->users[j]->group_info.group = group_id;
                    group_sta_for_each(group->users[j], tmp, map) {
                        if (group_id != tmp)
                            nb_users[tmp]--;
                    }
                }
            }
        }
    }
    mu->next_group_select = jiffies +
        msecs_to_jiffies(SIWIFI_MU_GROUP_SELECT_INTERVAL);
    mu->next_group_select |= 1;
}
